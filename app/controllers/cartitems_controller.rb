class CartitemsController < ApplicationController 
  include CurrentCart # mixin module
  before_action(:set_cart) # set cart returns a @cart object, session[:cart_id] store cart
  ...
  # come here if clicked 'Add to Cart'
  def create
    # @cartitem = Cartitem.new(cartitem_params)
    product_id = params[:product_id]
    product_obj = Product.find(product_id)
    # cart_id = 
    cart_obj = @cart #@cart is set by set_cart in CurrentCart module
    ...
    respond_to do |format|
      if @cartitem.save
        format.html { redirect_to @cart, notice: "Cartitem was successfully created." }
